$(document).ready(function(){
			...
			$.post(formURL, {
				...
				function(data){
					if(data == 0){
						...
						let json = JSON.parse(data);
						let text = (json.value == 1) ? '<span class="text-success">Còn hàng</span>' : '<span class="text-danger">Hết hàng</span>';
	...
	$(document).on('click','.td-show span', function(){
		let _this = $(this);
		let id = _this.parents('tr').attr('data-id');
		let field = _this.parent('td').attr('data-field');
		let $module = _this.parent('td').attr('data-module');
						...
						function(data){
							if(data == 0){
									...
									for(let i = 0; i < id.length; i++){
										let text = (param.value == 1) ? '<span class="text-success">Còn hàng</span>' : '<span class="text-danger">Hết hàng</span>';
					...
					$.post(formURL, {
						...
						function(data){
							if(data == 0){
									...
									for(let i = 0; i < id.length; i++){
										let text = (param.value == 1) ? '<span class="text-success">Conf hangf</span>' : '<span class="text-danger">Her hang</span>';
										$('#post-'+id[i]).find('.td-show').html(text);			
									}
									swal("Thành công!", "Thao tác thực hiện thành công!", "success");
								}
						});

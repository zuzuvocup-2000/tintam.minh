...
$(document).on('change', '.index_update_price', function(){
	let input = $(this).val();
	let originalInput = input.replace(/\./g, ''); // Loại bỏ dấu chấm để lấy giá trị số nguyên
	let val = parseInt(originalInput, 10);
	...
	$.post(form_URL, {
		...
		let json = JSON.parse(data);
		$(this).hide();
		$(this).siblings('.view_price').show();
		$(this).siblings('.view_price').html(json.val);
	}.bind(this)); 
});

...
$auth = (isset($_COOKIE[AUTH.'backend'])) ? json_decode($_COOKIE[AUTH.'backend'] , TRUE): '';
...
if(isset($auth['type']) && $auth['type'] == 'chutiem'){
	if (!file_exists($baseUrl . 'file'. $auth['id'])) mkdir($baseUrl . 'file'.'/file'. $auth['id'], 0777, true);
	if (!file_exists($baseUrl . 'image'. $auth['id'])) mkdir($baseUrl . 'image'.'/image'. $auth['id'], 0777, true);
	if (!file_exists($baseUrl . 'flash'. $auth['id'])) mkdir($baseUrl . 'flash'.'/flash'. $auth['id'], 0777, true);
	$config['ResourceType'][] = Array(
			'name' => 'Files',				// Single quotes not allowed
			'url' => $baseUrl . 'file'. '/file'. $auth['id'],
			'directory' => $baseDir . 'file'. '/file'. $auth['id'],
			'maxSize' => '1G',
			'allowedExtensions' => '7z,aiff,asf,avi,bmp,csv,doc,docx,fla,flv,gif,gz,gzip,jpeg,jpg,mid,mov,mp3,mp4,mpc,mpeg,mpg,ods,odt,pdf,png,svg,ppt,pptx,pxd,qt,ram,rar,rm,rmi,rmvb,rtf,sdc,sitd,swf,sxc,sxw,tar,tgz,tif,tiff,txt,vsd,wav,wma,wmv,xls,xlsx,zip',
			'deniedExtensions' => '');

	$config['ResourceType'][] = Array(
			'name' => 'Images',
			'url' => $baseUrl . 'image'. '/image'. $auth['id'],
			'directory' => $baseDir . 'image'. '/image'. $auth['id'],
			'maxSize' => 0,
			'allowedExtensions' => 'bmp,gif,jpeg,jpg,png,svg, webp, mp4',
			'deniedExtensions' => '');

	$config['ResourceType'][] = Array(
			'name' => 'Flash',
			'url' => $baseUrl . 'flash'. '/flash'. $auth['id'],
			'directory' => $baseDir . 'flash'. '/flash'. $auth['id'],
			'maxSize' => 0,

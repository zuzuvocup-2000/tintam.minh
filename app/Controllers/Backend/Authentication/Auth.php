			...
			$errorValidate = [
 		 	...
 		 	if ($this->validate($validate, $errorValidate)){
		 		$user = $this->AutoloadModel->_get_where([
		 			'table' => 'user',
		 			'select' => 'id, fullname, catalogueid, email, (SELECT permission FROM user_catalogue WHERE user_catalogue.id = user.catalogueid) as permission',
		 		...
		 		if($user['catalogueid'] == 7) $cookieAuth['type'] = 'chutiem';

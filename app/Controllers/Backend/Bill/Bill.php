...
class Bill extends BaseController{
	...
	public function index($page = 1){
		...
		if($config['total_rows'] > 0){
			...
			$this->data['billList'] = $this->AutoloadModel->_get_where([
				'select' => 'tb1.id, tb1.address, tb1.fullname, tb1.phone, tb1.email, tb1.total, tb1.created_at, tb1.quantity, tb1.method, tb1.status, tb1.messages, tb1.bill_id, tb1.company, tb3.name as city, tb1.nation, tb1.qrpost',
				'table' => $this->data['module'].' as tb1',
				'join' => [
					[
	...
	public function detail($id = 1){
		...
		$this->data['bill'] = $this->AutoloadModel->_get_where([
			...
			'join' => [
				[
					'vn_province as tb2','tb1.cityid = tb2.provinceid','left'
				],
				[
					'vn_district as tb3','tb1.districtid = tb3.districtid AND tb3.provinceid = tb1.cityid','left'
				],
			],
			'where' => [
				'tb1.id' => $id
		...
		$this->data['billDetail'] = $this->AutoloadModel->_get_where([
			'select' => 'tb1.product_id, tb1.name, tb1.price, tb1.quantity, tb1.subtotal, tb1.option, tb2.image',

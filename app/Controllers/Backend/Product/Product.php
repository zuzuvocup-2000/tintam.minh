...
class Product extends BaseController
{
	...
	public function __construct()
	{
		...
		$this->auth = (isset($_COOKIE[AUTH . 'backend'])) ? json_decode($_COOKIE[AUTH . 'backend'], TRUE) : '';
	...
	public function index($page = 1)
	{
		...
		$this->data['attribute_list'] = $this->AutoloadModel->_get_where([
			...
			'join' => [
				[
					'attribute_translate as tb2',
					'tb1.id = tb2.objectid AND tb2.module = "attribute" AND tb2.language = \'' . $this->currentLanguage() . '\'',
					'inner'
				]
			],
			'where' => [
	...
	private function condition_where()
	{
		...
		// if(isset($this->auth['type']) && $this->auth['type'] == 'chutiem'){
		// 	$where['tb1.userid_created'] = $this->auth['id'];
		// }
	...
	private function condition_join()
	{
		$join = [
			[
				...
				'tb1.id = tb2.objectid AND tb2.module = \'' . $this->data['module'] . '\' ',
				'inner'
			],
			[
			...
			[
				...
				'tb1.catalogueid = tb4.objectid AND tb4.module = "product_catalogue" AND tb3.language = \'' . $this->currentLanguage() . '\' ',
				'inner'
			],
		];
		$attr = $this->request->getGet('attr');
		if (isset($attr) && $attr != 0) {
			$join[] = [
				'attribute_relationship as tb5',
				'tb1.id = tb5.objectid AND tb2.module = \'' . $this->data['module'] . '\' ',
				'inner'
			];
		}
	...
	private function store($param = [])
	{
		...
		if (!empty($price_promotion) && is_string($price_promotion)) {
		...
		$store = [
			...
			'image' => $image,
			'time_end' => $end,
			'price_promotion' => str_replace('.', '', $this->request->getPost('price_promotion')),
	...
	private function export_brand()
	{
		$brand = $this->AutoloadModel->_get_where([
			...
			'join' =>  [
				[
					'brand_translate as tb2',
					'tb1.id = tb2.objectid AND tb2.module = "brand" AND tb2.language = \'' . $this->currentLanguage() . '\' ',
					'inner'
				],
			],
			'where' => [
		...
		if ($catalogueid > 0) {
			$catalogue = $this->AutoloadModel->_get_where([
				...
				'join' =>  [
					[
						'product_translate as tb3',
						'tb1.id = tb3.objectid AND tb3.language = \'' . $this->currentLanguage() . '\' ',
						'inner'
					],
				],
	...
	public function get_canonical()
	{
		$flag = $this->AutoloadModel->_get_where([
			...
			'join' => [
				[
					$this->data['module'] . '_translate as tb2',
					'tb1.id = tb2.objectid',
					'inner'
				]
			],
	...
	private function get_data_module($id = 0)
	{
		...
		$flag = $this->AutoloadModel->_get_where([
			...
			'join' =>  [
				[
					'product_translate as tb2',
					'tb1.id = tb2.objectid AND tb2.module = \'' . $this->data['module'] . '\' AND tb2.language = \'' . $this->currentLanguage() . '\'',
					'inner'
				]
			],
			'where' => $where

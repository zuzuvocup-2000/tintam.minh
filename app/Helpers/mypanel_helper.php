...
if (! function_exists('panel_post')){
    function panel_post($param = []){
        ...
        $data = $model->_get_where([
            'select' => 'tb1.id, tb2.title,tb2.module, tb1.image, tb1.catalogueid,tb2.description,tb2.content,tb1.userid_created,tb5.fullname, tb1.album,'.$param['select_post'].$iframe.',  tb2.title, tb2.meta_description, tb1.created_at, tb2.canonical, tb4.title as cat_title, tb4.canonical as cat_canonical',
...
if (! function_exists('post_and_cat')){
    function post_and_cat($param = []){
        ...
        $mang = $model->_get_where([
            'select' => 'tb1.catalogueid, tb3.id, tb2.module, '.$param['select_cat'].', tb2.title,tb2.description,  tb2.content, tb2.canonical, tb3.created_at, tb3.album,tb2.icon ,tb3.image, tb4.title as cat_title, tb4.canonical as cat_canonical, tb3.created_at',
